...
if login_result is not None:
    ...
    if auth_status:
        ...
        st.caption("Ascendra v1.2 is limited to CSV and PDF files. Selection of artefact types and taxonomies are not yet activated")
        ...
        if 'df_secondary' in locals() and isinstance(df_secondary, pd.DataFrame):
            ...
            if 'Primary_file' in st.session_state and 'Secondary_file' in st.session_state:
                if st.session_state['Primary_file'] and st.session_state['Secondary_file']:
                    ...
                    st.markdown("If you would like to use a specific taxonomy or classification system that can by used by Ascendra to refine the comparison procesas part of the comparative process, select one from the dropdown list below")
